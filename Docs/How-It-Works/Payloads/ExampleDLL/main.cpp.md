...
| `dllParam` | Stores the `HMODULE` (image base) passed to `DllMain`. Needed by `firstEntry` to resolve the DLL's own file path via `GetModuleFileNameA`. |
| `targetPath` | Hardcoded path of the process that `firstEntry` will inject this DLL into. Change this to retarget the injection. |
...
**Logic:**
- Stores `Base` in `dllParam` for later use by `firstEntry`.
...
> **Note:** `firstEntry` this name must match the `runningFunctionName` constant in `loadder.cpp` exactly (case-sensitive) for the LoadMe launcher to call it correctly.

The export invoked by **LoadMe** (`rundll32.exe … ,firstEntry`) when the DLL is first dropped into the user's profile. Its role is to identify the DLL's own location on disk and inject it into the hardcoded target process.
...
> **Dependency on `dllParam`:** This function relies on `DllMain` having been called first to populate `dllParam`. When launched via `rundll32.exe`, `DllMain` is called by the Windows loader before any export, so `dllParam` will be valid. When called from the injector's shellcode path (see `entryPoint`), `DllMain` is invoked by the shellcode immediately before `entryPoint` — not before `firstEntry` — so `dllParam` is also valid by the time `firstEntry` is called from `rundll32`.
...
**Remove the debug message box in `firstEntry`:**
